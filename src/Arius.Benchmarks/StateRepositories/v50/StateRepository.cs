...
namespace Arius.Benchmarks.StateRepositories.v50;
...
public class StateRepository
{
    ...
    internal BinaryProperties? GetBinaryProperty(Hash h)
    {
        using var context = GetContext();

        return context.BinaryProperties.SingleOrDefault(x => x.Hash == h);
    }

    internal void AddBinaryProperties(params BinaryProperties[] bps)
    {
        using var context = GetContext();
    ...
    internal void UpsertPointerFileEntries(params PointerFileEntry[] pfes)
    {
        using var context = GetContext();
    ...
    internal void DeletePointerFileEntries(Func<PointerFileEntry, bool> shouldBeDeleted)
    {
        using var context = GetContext();
