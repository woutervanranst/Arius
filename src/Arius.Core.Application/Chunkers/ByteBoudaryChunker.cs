...
public class ByteBoundaryChunker
{
    ...
    public ByteBoundaryChunker(int bufferSize = 1024 * 24,   // with an average chunk size of 14 KB at 4 KB min size, setting this sufficiently high enough to minimize allocations
    ...
    public IEnumerable<MemoryChunk> Chunk(Stream stream)
    {
        ...
        while (true)
        {
                ...
                subChunks.Sum(sc => sc.Length) + (i + Delimiter.Length - offset) >= MinChunkSize)  //the size of the chunk that will be made is large enough
            {
                ...
                yield return new MemoryChunk(chunk);
...
{
    public MemoryChunk(byte[] chunk) //TODO quid memory allocation??
    {
        Bytes = chunk;
    }
