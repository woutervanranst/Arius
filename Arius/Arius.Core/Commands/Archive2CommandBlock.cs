...
{
    internal class IndexDirectoryBlockFactory2 //: ISubject<DirectoryInfo, IFile>
    {
        public IndexDirectoryBlockFactory2(ILogger<IndexDirectoryBlockFactory2> logger)
        {
            this.logger = logger;

            Input = Observer.Create<DirectoryInfo>(
            onNext: (di) =>
            {
                p.OnNext(true);

                IndexDirectory(di, di).ToObservable().Subscribe(sf);
            },
            onError: (e) =>
            {
        ...
        private readonly ILogger<IndexDirectoryBlockFactory2> logger;



        public IObserver<DirectoryInfo> Input { get; init; }

        public IObservable<IFile> Output => sf;
        private readonly Subject<IFile> sf = new Subject<IFile>();


        public IObservable<bool> Progress => p;
        private readonly Subject<bool> p = new Subject<bool>();
        ...
        //public void OnNext(DirectoryInfo value)
        //{
        //    throw new NotImplementedException();
        //}

        //public IDisposable Subscribe(IObserver<IFile> observer)
        //{
        //    throw new NotImplementedException();
        //}
